...
:::note
The first row with 10 million partitions is surprisingly a lot faster than the
next row: I'm attributing this to the file system cache, which is able to keep
the entire index cached for the first run, but doesn't quite fit the second
row - and its effect is successively getting smaller the larger the data set.
:::
